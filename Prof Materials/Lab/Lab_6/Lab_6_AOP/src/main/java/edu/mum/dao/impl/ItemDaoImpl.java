...
/*		Query query = entityManager.createQuery("select i from Item i, Category c where c.name = :categoryName and i item of c.items");*/
	...
	public List<Item> findItemCriteria(Integer initialPrice, User buyer, User seller) {
		...
		query.select(itemRoot).distinct(true);
		 ...
		 Predicate sellerPricePredicate = criteriaBuilder.isTrue(init); 
		    ...
		    if ((seller != null) && (initialPrice != null)  ) {
		        ...
		        sellerPricePredicate = criteriaBuilder.or(and);
	    ...
	    if ((buyer != null)) {
		    	...
		    	Predicate andPrice = criteriaBuilder.and(memberOf,pricePredicate);
		    	Predicate andBuyer = criteriaBuilder.and(buyerPredicate,andPrice);
		    	buyerMatchPredicate = criteriaBuilder.or(andBuyer);
 		    }
		    
     		predicateList.add(criteriaBuilder.or(sellerPricePredicate,buyerMatchPredicate));
