 ...
 * NOTE: the BATCH Version is called ServiceValidationProcess
 */

@Component
public  class GenericValidator<T extends ValidationDto> {
	...
	CredentialsService credentialsService;
     ...
     public void  doValidate( ProceedingJoinPoint joinPoint, T validationDto) throws Throwable {
        ...
        Class<?> group = getValidationGroup(validationDto);
        ...
        Boolean validationSuccess = errors.size() == 0 ? true : false;       
   		...
   		newStatus = setProductStatus(validationDto); 
    	System.out.println("Validation Success! setting Status to: " + newStatus);

     
       	/*
       	 * Finished the "Before"
       	 * Now, Execute Lower priority Aspect &
       	 * THEN execute the joinpoint code
       	 * 
        ...
        */
      	...
      	if (validationSuccess) {
      		...
      		manageWorkQueue(validationDto,id,newStatus);
      	}
       return;
	...
	private void manageWorkQueue(T object, Long id, ProductionStatus status) {
    	UserCredentials owner = determineOwner(id, status);
